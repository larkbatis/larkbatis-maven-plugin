...
`mvn larkbatis:check` resolves through a group-level `maven-metadata.xml` that
...
```xml
<pluginGroups>
  <pluginGroup>io.github.larkbatis</pluginGroup>
</pluginGroups>
```
...
```bash
mvn io.github.larkbatis:larkbatis-maven-plugin:0.1.0:check
```
...
`gradle.properties` carries the version of `larkbatis-processor` that this
